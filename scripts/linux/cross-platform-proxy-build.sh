    ...
    do
        if [ $save_next_arg -eq 1 ]; then
            ...
            case "$arg" in
                ...
                "--project" ) save_next_arg=5;;
                "--configuration" ) save_next_arg=6;;
                "-h" | "--help" ) usage;;
                * ) usage;;
            esac
        fi
...
[[ -z "$PROJECT" ]] && { print_error 'Projects is a required parameter'; exit 1; }
...
case "$PACKAGE_OS.$PACKAGE_ARCH" in
    ubuntu18.04.amd64)
        ...
        SETUP_COMMAND=$'
            ...
            . ~/.cargo/env &&
        '
        CARGOFLAGS="$CARGOFLAGS --target $RUST_TARGET"
        TARGET="target/$RUST_TARGET/$BUILD_CONFIGURATION"
        STRIP_COMMAND=strip
        ;;
    ...
    alpine.amd64)
        ...
        SETUP_COMMAND=$'
            ...
            . ~/.cargo/env &&
        '
        CARGOFLAGS="$CARGOFLAGS --target x86_64-unknown-linux-musl"
        TARGET=target/x86_64-unknown-linux-musl/$BUILD_CONFIGURATION
        STRIP_COMMAND=strip   
        ;;
    ...
    ubuntu18.04.arm32v7)
        ...
        SETUP_COMMAND=$'
            ...
            . ~/.cargo/env &&
        '
        CARGOFLAGS="$CARGOFLAGS --target armv7-unknown-linux-gnueabihf"
        TARGET=target/armv7-unknown-linux-gnueabihf/$BUILD_CONFIGURATION
        STRIP_COMMAND=/usr/arm-linux-gnueabihf/bin/strip
        ;;
...
cd /tmp && \
    ...
    CARGOFLAGS="$CARGOFLAGS --target armv7-unknown-linux-musleabihf"
    TARGET=target/armv7-unknown-linux-musleabihf/$BUILD_CONFIGURATION
    STRIP_COMMAND=musl-strip
        ;;
    ...
    ubuntu18.04.aarch64| alpine.aarch64)
        ...
        SETUP_COMMAND=$'
            ...
            . ~/.cargo/env &&
        '
        CARGOFLAGS="$CARGOFLAGS --target aarch64-unknown-linux-gnu"
        TARGET=target/aarch64-unknown-linux-gnu/$BUILD_CONFIGURATION
        STRIP_COMMAND=/usr/aarch64-linux-gnu/bin/strip
        ;;
esac
...
CARGOFLAGS="$CARGOFLAGS --manifest-path=iotedged/Cargo.toml --no-default-features --features runtime-kubernetes $BUILD_CONFIG_OPTION -p $PROJECT "
BUILD_COMMAND="cargo build $CARGOFLAGS"
STRIP_COMMAND="$STRIP_COMMAND $TARGET/$PROJECT"
