...
build_project()
{
    ...
    if [[ "$ARCH" == "amd64" ]]; then
        ...
        docker run --rm -it -v "${API_PROXY_DIR}":/home/rust/src messense/rust-musl-cross:armv7-musleabihf  /bin/bash -c " rm -frv ~/.rustup/toolchains/* &&curl -sSLf https://sh.rustup.rs | sh -s -- -y && rustup target add armv7-unknown-linux-musleabihf && cargo build --target=armv7-unknown-linux-musleabihf --release --manifest-path /home/rust/src/Cargo.toml"
