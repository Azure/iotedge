...
echo "Configuring the docker daemon.json configuration file with IPv6 subnet $1..."
...
cat $DOCKER_CONFIG_PATH | jq -n --arg var1 "$1" '. + { "ipv6": true, "fixed-cidr-v6": "\($var1)" }' > $DOCKER_CONFIG_PATH
echo "Configured docker for IPv6."

echo "Configuring the sysctl config to allow router advertisements and enable ndp proxying on the network interface $3..."
...
SYSCTL_RA="net.ipv6.conf.$3.accept_ra=2"
SYSCTL_PNDP="net.ipv6.conf.$3.proxy_ndp=1"
