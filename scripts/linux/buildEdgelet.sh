...
build_project()
{
    ...
    execute cross build -p "$PROJECT" --manifest-path=$EDGELET_DIR/iotedged/Cargo.toml --no-default-features --features runtime-kubernetes "$BUILD_CONFIG_OPTION" --target "$TOOLCHAIN"
