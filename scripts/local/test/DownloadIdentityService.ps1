...
else
{
    ...
    $matches = Select-String -Path ./edgelet/Cargo.lock -Pattern `
        'github.com/nimanch/iot-identity-service\?branch=[ -~]+#(?<commit>\w+)' | `
...
for($page = 1; ; $page++)
{
    $url = "https://api.github.com/repos/nimanch/iot-identity-service/actions/runs?per_page=100&page=$page"
