...
foreach ($device in $devices)
{
    foreach ($alert in $alerts)
    {
        ...
        New-AzScheduledQueryRule -Location "West US 2" -ResourceGroupName "EdgeBuilds" -Action $alertingAction -Enabled $true -Description "$($device.deviceId) $($alert.Name) Threshold: $($alert.Threshold)" -Schedule $schedule -Source $querySource -Name $alertName
    }
}
