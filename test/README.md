...
| `hubResourceId` | * | Full path to Iot Hub that will receive the metrics messages in the following format - `/resource/subscriptions/<Azure subscription GUID>/resourceGroups/<resource group name>/providers/Microsoft.Devices/IotHubs/<Iot Hub name>`. Required when running the test 'AzureIotEdgeMetricsCollector', ignored otherwise.|
