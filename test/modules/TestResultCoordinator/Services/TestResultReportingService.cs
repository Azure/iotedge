    ...
    class TestResultReportingService : IHostedService, IDisposable
    {
        ...
        async void DoWorkAsync(object state)
        {
            ...
            if (this.serviceSpecificSettings.LogUploadEnabled)
            {
                try
                {
                    await TestReportUtil.UploadLogsAsync(Settings.Current.IoTHubConnectionString, this.serviceSpecificSettings.BlobStoreSasUri, this.logUploadDuration, this.logger);
                }
                catch (Exception ex)
                {
