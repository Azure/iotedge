...
namespace TestResultCoordinator.Reports.DirectMethod.Connectivity
{
    ...
    class NetworkStatusTimeline
    {
        ...
        public (NetworkControllerStatus networkControllerStatus, bool isWithinTolerancePeriod, TimeSpan TimeDiff) GetNetworkControllerStatusAndWithinToleranceAt(DateTime statusTime)
        {
            ...
            for (int i = 0; i < this.networkControllerTestResults.Count; i += 2)
            {
                ...
                if (statusTime > curr.CreatedAt)
                {
                    timeDiff = statusTime.Subtract(curr.CreatedAt);
                }
            }

            return (networkControllerStatus, isWithinTolerancePeriod, timeDiff);
        }
