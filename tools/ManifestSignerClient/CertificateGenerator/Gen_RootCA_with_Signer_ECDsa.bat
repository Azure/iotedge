...
openssl req -x509 -key %ROOT_PRIVATE_KEYNAME% -out %ROOT_PUBLIC_KEYNAME% -days 5000 %ROOT_SHA_ALGORITHM% -subj "/CN=Manifest Signer Client ECDSA Root CA"

openssl ecparam -genkey -name %SIGNER_KEY_ALGO% -out %SIGNER_PRIVATE_KEYNAME%

openssl req -out %SIGNER_CSR% -key %SIGNER_PRIVATE_KEYNAME% -new -days 365 %SIGNER_SHA_ALGORITHM% -subj "/CN=Manifest Signer Client ECDSA Signer Cert"
