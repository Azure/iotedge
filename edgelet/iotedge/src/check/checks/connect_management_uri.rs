...
impl Checker for ConnectManagementUri {
    ...
    fn description(&self) -> &'static str {
        "config.toml has correct URIs for daemon mgmt endpoint"
    }
...
impl ConnectManagementUri {
    fn inner_execute(&mut self, check: &mut Check) -> Result<CheckResult, failure::Error> {
        ...
        (scheme1, scheme2) if scheme1 != scheme2 => return Err(Context::new(
            format!(
                "config.toml has invalid combination of schemes for connect.management_uri ({:?}) and listen.management_uri ({:?})",
                scheme1, scheme2,
            ))
            .into()),
