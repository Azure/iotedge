...
fn analyze_cert_info(not_after: DateTime<Utc>, autogenerated_certs: bool) -> CheckResult {
    ...
    } else if not_after < now + Duration::weeks(26) {
        CheckResult::Warning(
            Context::new(format!(
            "The device CA cert will expire within the next 6 months, at {}. Renew the certificate before this date to retain functionality.\n\
            See https://aka.ms/iotedge-prod-checklist-certs for best practices.",
            not_after,
        ))
            .into(),
...
mod tests {
    ...
    use crate::check::{checks::certificates_quickstart::analyze_cert_info, CheckResult};
    ...
    fn production_certs_not_expired_is_ok() {
        let not_after = Utc::now() + Duration::weeks(30);
