    ...
    let edged_config = edgelet_docker::Settings {
        ...
        moby_runtime: {
            ...
            edgelet_docker::MobyRuntime {
                ...
                content_trust: content_trust.as_ref().map(|content_trust| -> Result<_, std::borrow::Cow<'static, str>> {
                    let old_config::ContentTrust { ca_certs } = content_trust;

                    Ok(edgelet_docker::ContentTrust {
                        ca_certs: ca_certs.as_ref().map(|ca_certs| -> Result<_, std::borrow::Cow<'static, str>> {
                            let mut new_ca_certs: std::collections::BTreeMap<_, _> = Default::default();

                            for (hostname, cert_path) in ca_certs {
                                let cert_id = format!("content-trust-{}", hostname);
                                let cert_uri = url::Url::from_file_path(cert_path).map_err(|()| format!("could not convert path {} to file URI", cert_path.display()))?;
                                certd_config.preloaded_certs.insert(cert_id.clone(), aziot_certd_config::PreloadedCert::Uri(cert_uri));
