        ...
        let response =
            ...
            .map(|(module_id, genid)| {
                ...
                req
                    ...
                    .concat2()
                    .then(|body| {
                        let body = body.context(ErrorKind::EncryptionOperation(EncryptionOperation::Decrypt))?;
                        Ok((id, body))
                    })
            })
            .into_future()
            ...
            .and_then(move |(id, body)| -> Result<_, Error> {
                let request: DecryptRequest = serde_json::from_slice(&body).context(ErrorKind::MalformedRequestBody)?;
                let ciphertext = base64::decode(request.ciphertext()).context(ErrorKind::MalformedRequestBody)?;
                let initialization_vector = base64::decode(request.initialization_vector()).context(ErrorKind::MalformedRequestBody)?;
                let plaintext = hsm.decrypt(id.as_bytes(), &ciphertext, &initialization_vector).context(ErrorKind::EncryptionOperation(EncryptionOperation::Decrypt))?;
                let encoded = base64::encode(&plaintext);
                let response = DecryptResponse::new(encoded);
                let body = serde_json::to_string(&response).context(ErrorKind::EncryptionOperation(EncryptionOperation::Decrypt))?;
