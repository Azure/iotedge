...
impl WorkloadService {
    pub fn new<M, W>(
        ...
        config: W,
    ) -> impl Future<Item = Self, Error = anyhow::Error> + 'static
    where
...
fn get_derived_identity_key_handle(
    identity_client: &Arc<Mutex<IdentityClient>>,
    name: &str,
) -> impl Future<Item = KeyHandle, Error = anyhow::Error> {
    ...
    id_client.get_module(name).then(|identity| match identity {
        Ok(aziot_identity_common::Identity::Aziot(spec)) => spec
            ...
            .with_context(|| anyhow::anyhow!("keyhandle missing"))
            .context(Error::GetIdentity),
        Ok(aziot_identity_common::Identity::Local(_)) => {
            Err(anyhow::anyhow!(Error::InvalidIdentityType))
        }
        Err(err) => Err(anyhow::anyhow!(err).context(Error::GetIdentity)),
    })
}
...
fn get_master_encryption_key(
    ...
    key_client
        ...
        .map_err(|_| anyhow::anyhow!(Error::LoadMasterEncKey))
        .into_future()
}
