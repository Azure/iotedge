...
use edgelet_core::{
    Authenticator, MakeModuleRuntime, Module, ModuleAction, ModuleRuntime,
...
// This is the name of the directory that contains modules sockets
    ...
    S: RuntimeSettings,
{
    ...
    env.insert(
        ...
        settings.connect().workload_socket_uri().to_string(),
    );
    env.insert(MANAGEMENT_URI_KEY.to_string(), management_uri);
    env.insert(
        WORKLOAD_LISTEN_MNT_URI.to_string(),
        settings.listen().workload_mnt_uri(&home_dir),
    );
