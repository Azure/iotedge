...
mkdir -p $BUILD_DIR
COMMAND="cd /project/edgelet && make deb VERSION=${VERSION} CARGOFLAGS=\"--target aarch64-unknown-linux-gnu\" TARGET=target/aarch64-unknown-linux-gnu/release DPKGFLAGS=\"-b -us -uc -i --host-arch arm64 --host-type aarch64-linux-gnu --target-type aarch64-linux-gnu\""
