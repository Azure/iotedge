...
function Get-OpenSSL
{
    ...
    if (!((Test-Path -Path $env:SystemDrive\vcpkg) -and ((Test-Path -Path $env:SystemDrive\vcpkg\vcpkg.exe))))
    {
        Write-Host "Installing vcpkg from github..."
        git clone https://github.com/Microsoft/vcpkg $env:SystemDrive\vcpkg
        if ($LastExitCode)
        {
            Throw "Failed to clone vcpkg repo with exit code $LastExitCode"
        }
        Write-Host "Bootstrapping vcpkg..."
        & "$env:SystemDrive\vcpkg\bootstrap-vcpkg.bat"
        if ($LastExitCode)
        {
            Throw "Failed to bootstrap vcpkg with exit code $LastExitCode"
        }
        Write-Host "Installing vcpkg..."
        & $env:SystemDrive\\vcpkg\\vcpkg.exe integrate install
        if ($LastExitCode)
        {
    ...
    if (!(Test-Path -Path $env:SystemDrive\vcpkg\Downloads))
    {
    ...
    if ((Test-Path env:TF_BUILD) -and ($env:TF_BUILD -eq $true))
    {
        ...
        Write-Host "##vso[task.setvariable variable=OPENSSL_ROOT_DIR;]$env:SystemDrive\vcpkg\installed\$(if($Arm){ "arm" }else{ "x64"})-windows"
        # Rust's openssl-sys crate needs this environment set.
        Write-Host "##vso[task.setvariable variable=OPENSSL_DIR;]$env:SystemDrive\vcpkg\installed\$(if($Arm){ "arm" }else{ "x64"})-windows"
    }
    else
    {
        # for local installation, set the env variable within the USER scope
        Write-Host "Local installation detected"
        [System.Environment]::SetEnvironmentVariable("OPENSSL_ROOT_DIR", "$env:SystemDrive\vcpkg\installed\$(if($Arm){ "arm" }else{ "x64"})-windows", [System.EnvironmentVariableTarget]::User)
        [System.Environment]::SetEnvironmentVariable("OPENSSL_DIR", "$env:SystemDrive\vcpkg\installed\$(if($Arm){ "arm" }else{ "x64"})-windows", [System.EnvironmentVariableTarget]::User)
    }
