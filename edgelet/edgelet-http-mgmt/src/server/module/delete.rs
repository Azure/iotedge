        ...
        let response =
            ...
            .map(|name| {
                ...
                self
                    ...
                    .then(|result| match result {
                        Ok(_) => Ok(name),
                        Err(err) => Err(Error::from(err.context(ErrorKind::RuntimeOperation(RuntimeOperation::RemoveModule(name))))),
                    })
            })
            .into_future()
            .flatten()
            .and_then(|name| {
                Ok(Response::builder()
                    ...
                    .body(Body::default())
                    .context(ErrorKind::RuntimeOperation(RuntimeOperation::RemoveModule(name)))?)
            })
