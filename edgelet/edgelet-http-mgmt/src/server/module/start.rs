    ...
    fn handle(
        ...
        let response = params
            ...
            .map(|name| {
                ...
                self.runtime.start(&name).then(|result| {
                    result
                        .with_context(|| {
                            Error::RuntimeOperation(RuntimeOperation::StartModule(name.clone()))
                        })
                        .map(|_| name)
                })
            })
            ...
            .and_then(|name| {
                Ok(Response::builder()
                    ...
                    .body(Body::default())
                    .context(Error::RuntimeOperation(RuntimeOperation::StartModule(name)))?)
            })
