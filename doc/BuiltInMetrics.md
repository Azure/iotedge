...
| `edgehub_twin_signatures_checks_count` | `result` | The number of twin signature checks, `result` is "Success" or "Failure". | Counter |
| `edgehub_twin_signatures_checks_seconds` |  | The amount of time it took to verify twin signatures. | Counter |
...
| `edgeAgent_manifest_integrity_flag` | `signing_with_ca_enabled`, `signing_with_integrity_enabled` | The value is 0 (no manifest integrity) or 1 (manifest integrity is present), tags are "true" or "false" and indicate which integrity components are present. | Gauge |
| `edgeAgent_twin_signatures_checks_count` | `result` | The number of twin signature checks, `result` is "Success" or "Failure". | Counter |
| `edgeAgent_twin_signatures_checks_seconds` |  | The amount of time it took to verify twin signatures. | Counter |
|||
