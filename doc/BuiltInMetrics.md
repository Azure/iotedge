...
| `edgehub_messages_ttl_cleanup_count`                               |  `endpoint` (Message source)<br>                                                          | Total number of messages removed because TTL expired                                                                                                                                                                        | counter |
| `edgehub_offline_count_total`                               |                                                          | Total number of times edgeHub went offline                                                                                                                                                                        | counter | 
| `edgehub_offline_duration_seconds`                                |  <br> `quantile`(Percentile [50, 90, 95, 99, 99.9, 99.99]) | P50, P90, P95, P99, P99.9 and P99.99 time edge hub was offline. Values may be reported as `NaN`  if no new measurements are reported for a  certain period of time (currently 10 minutes). As this is `summary` type, corresponding `_count` and `_sum` counters will be emitted.              | summary |
| `edgehub_operation_retry_total`      |                                                          | Total number of times edgeHub operations were retried                                                                                                                                                                        | counter |                                                              
