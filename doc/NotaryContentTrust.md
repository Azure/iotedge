# **Notary Content Trust for Manifest Trust in IoT Edge**
...
1. **Set up the necesarry certificates**
    * Install the tools
    * Generate certificates
        * Create the root CA thats pins to the edge device 
        * Create the root certificates for each container images that chains to the root CA
2. **Initialize TUF Trust Collection using Notary client**
    * Notary Installation
    * Notary Configuration
    * Notary TUF Trust collection Intialization
    * Notary Snapshot key rotation
3. **Sign and publish images using Docker client**
    * Docker Environment variable configuration
    * Sign and publish 
4. **Enable Content Trust in IoT edge device**
    
## Step 1. Set up the necessary certificates 
...
**Note:**
root CA per registry is a scoping choice in this experimental release. That a full-fledged feature would consider consolidating per registry roots into a unified root.
##### c. Steps to create root CA for a registry that pins to the Edge device
...
##### d. Steps to create root certificates for each container image
...
-  The Common Name (CN) in root certificate for each container must contain the Globally Unique Names(GUNs) i.e exampleregistry.azurecr.io/image 
-  Safely store the pass code for the private key for root_ca_exampleregistry.key and root_image.key. These will be used in the next steps.	
...
##### c. Notary TUF Trust collection initialization
...
##### d. Notary Snapshot key rotation 
...
## Step 3. Sign and publish images using Docker Client
...
##### a.  Docker Environment variable configuration
...
## Step 4. Enable Content Trust in IoT edge device
