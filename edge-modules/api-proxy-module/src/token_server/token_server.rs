    ...
    let token_server: JoinHandle<Result<()>> = tokio::spawn(async move {
        ...
        loop {
            ...
            let make_svc = make_service_fn(move |_conn| {
                ...
                async move { 
                    Ok::<_, Error>(service_fn( move |req| server_callback(req, token_client_clone.clone())))            
                }
            });
        
            let addr = ([127, 0, 0, 1], 6001).into();
            let server = Server::bind(&addr).serve(make_svc);
            futures::pin_mut!(wait_shutdown);
            futures::pin_mut!(server);

            match futures::future::select(wait_shutdown, server).await {
                Either::Left(_) => {
...
async fn server_callback(_: Request<Body>, token_client: Arc<token_client::TokenClient>) -> Result<Response<Body>, Error> {
    ...
    let token = token_client.get_new_sas_token(&token_renewal_time.timestamp().to_string())
    .await?;
