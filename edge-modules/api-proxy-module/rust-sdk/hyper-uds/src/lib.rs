            ...
            move || -> std::io::Result<()> {
                'outer: while let Ok(waker) = flush_receiver.recv() {
                    loop {
                        ...
                        let result = pending_write_bytes.map_or(Ok(()), |pending_write_bytes| {
        ...
        Box::pin(async move {
            ...
            if scheme != Some("unix") {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    format!(
                        r#"expected req to have "unix" scheme but it has {:?}"#,
                        scheme
                    ),
                ));
            }
            ...
            let path: std::path::PathBuf = host
                .ok_or_else(|| format!("could not decode UDS path from req host {:?}", host))
                .and_then(|host| {
                    hex::decode(host).map_err(|err| {
                        format!(
                            "could not decode UDS path from req host {:?}: {}",
                            host, err
                        )
                    })
                })
                .and_then(|path| {
                    String::from_utf8(path).map_err(|err| {
                        format!(
                            "could not decode UDS path from req host {:?}: {}",
                            host, err
                        )
...
pub fn make_hyper_uri(
    base: &str,
    path: &str,
) -> Result<hyper::Uri, <hyper::Uri as std::str::FromStr>::Err> {
