                ...
                State::Idle => {
                        ...
                        .poll_next(cx)
                    {
                        ...
                        std::task::Poll::Ready(Some(Ok(mqtt3::Event::Publication(
                            publication,
                        )))) => match InternalMessage::parse(publication, &this.c2d_prefix) {
                            Ok(InternalMessage::CloudToDevice(message)) => {
                                return std::task::Poll::Ready(Some(Ok(Message::CloudToDevice(
                                    message,
                                ))))
                            }
...
impl std::fmt::Display for MessageParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MessageParseError::C2DMessageMissingRequiredProperty(property) => write!(
                f,
                "C2D message does not contain required property {}",
                property
            ),
...
impl InternalMessage {
    fn parse(
        ...
        if publication.topic_name.starts_with(c2d_prefix) {
            ...
            for (key, value) in
                url::form_urlencoded::parse(publication.topic_name[c2d_prefix.len()..].as_bytes())
            {
            ...
            let system_properties = system_properties
                .build()
                .map_err(MessageParseError::C2DMessageMissingRequiredProperty)?;
