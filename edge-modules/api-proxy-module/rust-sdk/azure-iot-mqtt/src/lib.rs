...
mod io;
pub use io::{Io, IoSource, Transport, IOTHUB_ENCODE_SET};
...
impl std::fmt::Display for CreateClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CreateClientError::InvalidDefaultSubscription(err) => {
                write!(f, "could not create default MQTT subscription: {}", err)
            }
            CreateClientError::ParseEnvironmentVariable(name, err) => {
                write!(f, "could not parse environment variable {}: {}", name, err)
            }
...
impl DirectMethodResponseHandle {
    ...
    pub async fn respond(
    ...
    ) -> Result<(), DirectMethodResponseError> {
        use futures_util::SinkExt;

        let (ack_sender, ack_receiver) = futures_channel::oneshot::channel();

        let direct_method_response = DirectMethodResponse {
            request_id,
            status,
            payload,
            ack_sender,
        ...
        self.0
            .send(direct_method_response)
            .await
            .map_err(|_| DirectMethodResponseError::ClientDoesNotExist)?;
        let publish = ack_receiver
            .await
            .map_err(|_| DirectMethodResponseError::ClientDoesNotExist)?;
        publish
            .await
...
struct DirectMethodResponse {
    ...
    ack_sender: futures_channel::oneshot::Sender<
        Box<dyn Future<Output = Result<(), mqtt3::PublishError>> + Send + Unpin>,
    >,
}
    ...
    for subscribe_to in default_subscriptions {
        match inner.subscribe(subscribe_to) {
            ...
            Err(err @ mqtt3::UpdateSubscriptionError::EncodePacket(..)) => {
                return Err(CreateClientError::InvalidDefaultSubscription(err))
            }
        }
    }
