...
#[derive(Debug)]
enum State {
    WaitingForSubscriptions { reset_session: bool, acked: usize },
...
impl Client {
    ...
    pub fn new_for_edge_module(
        ...
        let device_id = std::env::var("IOTEDGE_DEVICEID").map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable("IOTEDGE_DEVICEID", Box::new(err))
        })?;

        let module_id = std::env::var("IOTEDGE_MODULEID").map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable("IOTEDGE_MODULEID", Box::new(err))
        })?;

        let generation_id = std::env::var("IOTEDGE_MODULEGENERATIONID").map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable(
                "IOTEDGE_MODULEGENERATIONID",
                Box::new(err),
            )
        })?;

        let edgehub_hostname = std::env::var("IOTEDGE_GATEWAYHOSTNAME").map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable(
        ...
        let iothub_hostname = std::env::var("IOTEDGE_IOTHUBHOSTNAME").map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable(
        ...
        let workload_url = std::env::var("IOTEDGE_WORKLOADURI").map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable("IOTEDGE_WORKLOADURI", Box::new(err))
        })?;
        let workload_url = workload_url.parse().map_err(|err| {
            crate::CreateClientError::ParseEnvironmentVariable("IOTEDGE_WORKLOADURI", Box::new(err))
        })?;
