...
#[derive(Debug, serde_derive::Deserialize, serde_derive::Serialize, Default)]
    ...
    pub(crate) fn parse(
        ...
        if let Some(captures) = RESPONSE_REGEX.captures(&publication.topic_name) {
            ...
            let status: crate::Status = status
                .parse()
                .map_err(|err| MessageParseError::ParseResponseStatus(status.to_string(), err))?;
            ...
            Ok(InternalTwinStateMessage::Response {
                status,
                request_id,
                version,
...
impl std::fmt::Display for MessageParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MessageParseError::IotHubStatus(status) => {
                write!(f, "IoT Hub failed request with status {}", status)
            }
            MessageParseError::Json(err) => {
                write!(f, "could not parse payload as valid JSON: {}", err)
            }
            MessageParseError::MissingResponseRequestId => {
                write!(f, r#"could not parse response: missing "$rid" property"#)
            }
            MessageParseError::ParseResponseVersion(version, err) => write!(
                f,
                "could not parse {:?} as version number of reported twin state: {}",
                version, err
            ),
            MessageParseError::ParseResponseRequestId(request_id, err) => {
                write!(f, "could not parse {:?} as request ID: {}", request_id, err)
            }
            MessageParseError::ParseResponseStatus(status, err) => {
                write!(f, "could not parse {:?} as status code: {}", status, err)
            }
            MessageParseError::UnrecognizedMessage(publication) => write!(
                f,
                "message with topic {:?} could not be recognized",
