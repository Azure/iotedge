...
* Added Microsoft Entra ID authentication via an explicit `ChainedTokenCredential` (certificate, client secret, workload identity federation, managed identity), configured through the standard `AZURE_*` environment variables. Unlike `DefaultAzureCredential`, developer-machine credentials (Azure CLI, Visual Studio, etc.) are intentionally excluded so they can't silently substitute for the intended identity in production.
