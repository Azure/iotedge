...
namespace Microsoft.Azure.Devices.Edge.Agent.IoTHub
{
    ...
    public class ModuleClient2 : IModuleClient
    {
        ...
        ModuleClient2(Client.ModuleClient moduleClient)
        {
        ...
        public static Task<IModuleClient> Create(
            ...
            Func<ModuleClient2, Task> initialize,
        ...
        static async Task<IModuleClient> Create(Option<UpstreamProtocol> upstreamProtocol, Func<ModuleClient2, Task> initialize, Func<UpstreamProtocol, Task<Client.ModuleClient>> deviceClientCreator)
        {
            try
            {
                return await ExecuteWithRetry(
                    async () =>
                    {
                        Client.ModuleClient mc = await CreateDeviceClientForUpstreamProtocol(upstreamProtocol, deviceClientCreator);
                        Events.DeviceClientCreated();
                        var moduleClient = new ModuleClient2(mc);
                        await (initialize?.Invoke(moduleClient) ?? Task.CompletedTask);
                        return moduleClient;
                    },
            ...
            Option<string> productInfo)
        {
            ...
            Client.ModuleClient moduleClient = await connectionString
            ...
            productInfo.ForEach(p => moduleClient.ProductInfo = p);

            // note: it's important to set the status-changed handler and
            // timeout value *before* we open a connection to the hub
            moduleClient.OperationTimeoutInMilliseconds = DeviceClientTimeout;
