    ...
    class ModuleManagementHttpClient : ModuleManagementHttpClientVersioned
    {
        ...
        public override async Task<SystemInfo> GetSystemInfoAsync(CancellationToken cancellationToken)
        {
            using (HttpClient httpClient = this.GetHttpClient())
            {
                ...
                return new SystemInfo(systemInfo.OsType, systemInfo.Architecture, systemInfo.Version, provisioning, systemInfo.Server_version, systemInfo.Kernel_version, systemInfo.Operating_system, systemInfo.Cpus ?? 0, /* totalMemory */ 0, systemInfo.Virtualized, new Dictionary<string, object>());
            }
        }
