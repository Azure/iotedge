...
namespace Microsoft.Azure.Devices.Edge.Agent.Core.Test
{
    ...
    public class TestModuleBase<TConfig> : IModule<TConfig>
    {
        ...
        public bool OnlyModuleStatusChanged(IModule other) =>
            ...
            this.PullPolicy == other.PullPolicy;
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                hashCode = (hashCode * 397) ^ this.PullPolicy.GetHashCode();
                return hashCode;
            }
        }
    }
    ...
    public class TestModule : TestModuleBase<TestConfig>
    {
        public TestModule(
            ...
            ConfigurationInfo configuration,
            IDictionary<string, EnvVal> env)
            : base(name, version, type, desiredStatus, config, restartPolicy, pullPolicy, configuration, env)
        {
        }

        public TestModule CloneWithImage(string image)
        {
            return new TestModule(this.Name, this.Version, this.Type, this.DesiredStatus, new TestConfig(image), this.RestartPolicy, this.PullPolicy, this.ConfigurationInfo, this.Env);
        }
    }

    public class TestAgentModule : TestModule, IEdgeAgentModule
    {
        public TestAgentModule(string name, string type, TestConfig config, PullPolicy pullPolicy, ConfigurationInfo configuration, IDictionary<string, EnvVal> env)
            : base(name ?? Constants.EdgeAgentModuleName, string.Empty, type, ModuleStatus.Running, config, RestartPolicy.Always, pullPolicy, configuration, env)
        {
    ...
    {
        public TestHubModule(string name, string type, ModuleStatus desiredStatus, TestConfig config, RestartPolicy restartPolicy, PullPolicy pullPolicy, ConfigurationInfo configuration, IDictionary<string, EnvVal> env)
            : base(name ?? Constants.EdgeHubModuleName, string.Empty, type, desiredStatus, config, restartPolicy, pullPolicy, configuration, env)
        {
            this.Version = string.Empty;
        }
