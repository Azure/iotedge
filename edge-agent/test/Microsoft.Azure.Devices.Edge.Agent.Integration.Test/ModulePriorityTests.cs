...
namespace Microsoft.Azure.Devices.Edge.Agent.Integration.Test
{
    ...
    public class ModulePriorityTests
    {
        ...
        {
            Type agentTestsType = typeof(ModulePriorityTests);
            string format = $"{agentTestsType.Namespace}.{{0}}, {agentTestsType.Assembly.GetName().Name}";

            string json = File.ReadAllText("module-priority-test-config.json");
            ...
            //ISerde<TestConfig[]> serde = new TypeSpecificSerDe<TestConfig[]>(deserializerTypes);

            //return serde.Deserialize<TestConfig[]>(json).Select(config => new object[] { config });
        ...
        public async Task ModulesProcessedInPriorityOrder(TestConfig testConfig)
        {
            try
            {
                ...
                {
                    validated = testConfig.Validator.Validate(commandFactory);
                    if (!validated)
                    {
