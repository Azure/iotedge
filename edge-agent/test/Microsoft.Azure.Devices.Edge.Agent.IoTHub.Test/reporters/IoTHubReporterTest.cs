    ...
    public class IoTHubReporterTest
    {
        ...
        public async void ReportShutdown()
        {
            using (var cts = new CancellationTokenSource(Timeout))
            {
                ...
                IEdgeAgentModule edgeAgent = new EdgeAgentDockerRuntimeModule(new DockerReportedConfig("image", string.Empty, "hash", Option.None<NotaryContentTrust>()), ModuleStatus.Running, 0, string.Empty, lastStartTimeUtc, DateTime.MinValue, ImagePullPolicy.OnCreate, new ConfigurationInfo("id"), new Dictionary<string, EnvVal>());
                IEdgeHubModule edgeHub = new EdgeHubDockerRuntimeModule(ModuleStatus.Running, RestartPolicy.Always, new DockerReportedConfig("hubimage", string.Empty, "hash", Option.None<NotaryContentTrust>()), 0, string.Empty, DateTime.Now, DateTime.Now, 0, DateTime.Now, ModuleStatus.Running, ImagePullPolicy.OnCreate, Constants.DefaultStartupOrder, new ConfigurationInfo("hub"), new Dictionary<string, EnvVal>());
