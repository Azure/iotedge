...
namespace Microsoft.Azure.Devices.Edge.Agent.Kubernetes.Test
{
    ...
    public class KubernetesServiceMapperTest
    {
        ...
        public void EmptyIsNotAllowedAsServiceAnnotation()
        {
            ...
            var docker = new DockerModule(moduleId.ModuleId, "v1", ModuleStatus.Running, RestartPolicy.Always, Config1, ImagePullPolicy.OnCreate, Constants.DefaultPriority, DefaultConfigurationInfo, EnvVars, Secrets);
        ...
        public void NoPortsExposedMeansNoServiceCreated()
        {
            ...
            var config = new KubernetesConfig("image", createOptions, Option.None<AuthConfig>());
            var moduleId = new ModuleIdentity("hub", "gateway", "deviceId", "moduleid", Mock.Of<ICredentials>());
            var docker = new DockerModule("module1", "v1", ModuleStatus.Running, RestartPolicy.Always, Config1, ImagePullPolicy.OnCreate, Constants.DefaultPriority, DefaultConfigurationInfo, EnvVars, Secrets);
            var module = new KubernetesModule(docker, config, EdgeletModuleOwner);
            var moduleLabels = new Dictionary<string, string>();
