...
{
    $toolchain = Get-Content -Encoding UTF8 (Join-Path -Path (Get-MqttFolder) -ChildPath 'rust-toolchain')
...
function Get-Manifest
{
    ...
    Join-Path -Path $ProjectRoot -ChildPath "mqtt/Cargo.toml"
}

function Get-MqttFolder
{
    $ProjectRoot = Join-Path -Path $PSScriptRoot -ChildPath "../../.."
    Join-Path -Path $ProjectRoot -ChildPath "mqtt"
}

function Get-IotEdgeFolder
{
    # iotedge is parent folder of mqtt
    Join-Path -Path $(Get-MqttFolder) -ChildPath ".."
}
