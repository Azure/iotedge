...
namespace Microsoft.Azure.Devices.Edge.Hub.Core.Test.Routing
{
    ...
    public class RoutingTest
    {
        ...
        static async Task<(IEdgeHub, IConnectionManager)> SetupEdgeHub(IEnumerable<string> routes, IoTHub iotHub, string edgeDeviceId)
        {
            ...
            IConnectionManager connectionManager = new ConnectionManager(cloudConnectionProvider.Object, Mock.Of<ICredentialsCache>(), new IdentityProvider(iotHubName, Option.None<string>()), deviceConnectivityManager);
        ...
        static IClientCredentials SetupDeviceIdentity(string deviceId) =>
            new TokenCredentials(new DeviceIdentity("iotHub", Option.None<string>(), deviceId), Guid.NewGuid().ToString(), string.Empty, false);

        static IClientCredentials SetupModuleCredentials(string moduleId, string deviceId) =>
            new TokenCredentials(new ModuleIdentity("iotHub", Option.None<string>(), deviceId, moduleId), Guid.NewGuid().ToString(), string.Empty, false);
