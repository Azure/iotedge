...
namespace Microsoft.Azure.Devices.Edge.Hub.Core.Test.Routing
{
    ...
    public class RoutingTest
    {
        ...
        static async Task<(IEdgeHub, IConnectionManager)> SetupEdgeHub(IEnumerable<string> routes, IoTHub iotHub, string edgeDeviceId)
        {
            ...
            ITwinManager twinManager = new TwinManager(connectionManager, new TwinCollectionMessageConverter(), new TwinMessageConverter(), Option.None<IEntityStore<string, Twin>>());
            IEdgeHub edgeHub = new RoutingEdgeHub(router, routingMessageConverter, connectionManager, twinManager);
