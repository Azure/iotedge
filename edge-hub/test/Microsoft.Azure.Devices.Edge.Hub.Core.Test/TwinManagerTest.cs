namespace Microsoft.Azure.Devices.Edge.Hub.Core.Test
{
    ...
    public class TwinManagerTest
    {
        ...
        [Fact]
        public void ComputeDiffSuccess()
        {
            // Arrange
            TwinCollection baseline = new TwinCollection()
            {
            ...
            TwinCollection patch = new TwinCollection()
            {
                ...
                {
                    //["level0"] = "nochange", // unchanged
                    ["level1"] = null, // existing in base. remove property
                    ["level2"] = new TwinCollection()
                    {
                        ["level3"] = "newvalue3" // existing in base, update property
                    },
            ...
            // Act
            TwinCollection resultCollection = TwinManager.DiffTwinCollections(baseline, mergedExcludeNull);

            // Assert
            Assert.True(JToken.DeepEquals(JsonConvert.DeserializeObject<JToken>(resultCollection.ToJson()), JsonConvert.DeserializeObject<JToken>(patch.ToJson())));

            // Act
            resultCollection = TwinManager.DiffTwinCollections(baseline, baseline);

            // Assert
            Assert.True(JToken.DeepEquals(JsonConvert.DeserializeObject<JToken>(resultCollection.ToJson()), JsonConvert.DeserializeObject<JToken>(emptyPatch.ToJson())));

            // Act
            resultCollection = TwinManager.DiffTwinCollections(emptyBaseline, emptyBaseline);

            // Assert
            Assert.True(JToken.DeepEquals(JsonConvert.DeserializeObject<JToken>(resultCollection.ToJson()), JsonConvert.DeserializeObject<JToken>(emptyPatch.ToJson())));

            // Act
            resultCollection = TwinManager.DiffTwinCollections(emptyBaseline, mergedExcludeNull);
