...
namespace Microsoft.Azure.Devices.Edge.Hub.CloudProxy.Test
{
    ...
    public class DeviceConnectivityManagerTest
    {
        ...
        public async Task NoEventsTest()
        {
            ...
            ICloudProxy cloudProxy = new CloudProxy(connectivityAwareClient, Mock.Of<IMessageConverterProvider>(), "d1/m1", null, Mock.Of<ICloudListener>(), TimeSpan.FromHours(1), true, TimeSpan.FromSeconds(50));
        ...
        public async Task WithDownstreamEventsTest()
        {
            ...
            ICloudProxy cloudProxy = new CloudProxy(edgeHubClient, Mock.Of<IMessageConverterProvider>(), "d1/m1", null, Mock.Of<ICloudListener>(), TimeSpan.FromHours(1), true, TimeSpan.FromSeconds(50));
            var connectionManager = Mock.Of<IConnectionManager>(c => c.GetCloudConnection("d1/m1") == Task.FromResult(Option.Some(cloudProxy)));
            deviceConnectivityManager.SetConnectionManager(connectionManager);
