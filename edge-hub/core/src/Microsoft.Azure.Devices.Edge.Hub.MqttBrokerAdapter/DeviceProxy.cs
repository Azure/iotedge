...
namespace Microsoft.Azure.Devices.Edge.Hub.MqttBrokerAdapter
{
    ...
    public class DeviceProxy : IDeviceProxy
    {
        ...
        public INestingInfo NestingInfo { get; }
                    ...
                    IDirectMethodHandler directMethodHandler)
        {
            ...
            this.NestingInfo = new NestingInfo(isDirectClient);
        ...
        public Task<DirectMethodResponse> InvokeMethodAsync(DirectMethodRequest request)
        {
            ...
            return this.directMethodHandler.CallDirectMethodAsync(request, this.Identity, this.NestingInfo.IsDirectClient);
        }

        public Task OnDesiredPropertyUpdates(IMessage desiredProperties)
        {
            Events.SendingDesiredPropertyUpdate(this.Identity);
            return this.twinHandler.SendDesiredPropertiesUpdate(desiredProperties, this.Identity, this.NestingInfo.IsDirectClient);
        }

        public Task SendC2DMessageAsync(IMessage message)
        {
            Events.SendingC2DMessage(this.Identity);
            return this.cloud2DeviceMessageHandler.SendC2DMessageAsync(message, this.Identity, this.NestingInfo.IsDirectClient);
        }

        public Task SendMessageAsync(IMessage message, string input)
        {
            Events.SendingModuleToModuleMessage(this.Identity);
            return this.moduleToModuleMessageHandler.SendModuleToModuleMessageAsync(message, input, this.Identity, this.NestingInfo.IsDirectClient);
        }

        public Task SendTwinUpdate(IMessage twin)
        {
            Events.SendingTwinUpdate(this.Identity);
            return this.twinHandler.SendTwinUpdate(twin, this.Identity, this.NestingInfo.IsDirectClient);
        }
