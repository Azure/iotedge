    ...
    public class CloudConnectionProvider : ICloudConnectionProvider
    {
        ...
        public async Task<Try<ICloudConnection>> Connect(IClientCredentials clientCredentials, Action<string, CloudConnectionStatus> connectionStatusChangedHandler)
        {
            ...
            try
            {
                ...
                string productInfo = await this.metadataStore.GetEdgeProductInfo(clientCredentials.Identity.Id);
                Option<string> modelId = await this.metadataStore.GetModelId(clientCredentials.Identity.Id);
                ...
                return await serviceIdentity
                    .Map(
                        async si =>
                        {
                            ...
                            string productInfo = await this.metadataStore.GetEdgeProductInfo(identity.Id);
                            Option<string> modelId = await this.metadataStore.GetModelId(identity.Id);
                            ICloudConnection cc = await CloudConnection.Create(
                                identity,
                                connectionStatusChangedHandler,
                                this.transportSettings,
