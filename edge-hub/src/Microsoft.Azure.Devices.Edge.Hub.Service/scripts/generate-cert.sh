...
if [ ! -f $SSL_CERTIFICATE_PATH/mqtt-server.pfx ] ; then
 ...
 openssl pkcs12 -export -out $SSL_CERTIFICATE_PATH/mqtt-server.pfx -inkey /etc/ssl/private/mqtt-server.key -in $SSL_CERTIFICATE_PATH/mqtt-server.crt -passout pass:
