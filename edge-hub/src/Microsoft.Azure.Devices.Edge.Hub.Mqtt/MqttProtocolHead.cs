    ...
    public class MqttProtocolHead : IProtocolHead
    {
        ...
        public async Task CloseAsync(CancellationToken token)
        {
            try
            {
                ...
                try
                {
                    await TaskEx.TimeoutAfter(this.parentEventLoopGroup?.ShutdownGracefullyAsync(), TimeSpan.FromSeconds(15));
                }
                catch (TimeoutException ex)
                {
                ...
                try
                {
                    await TaskEx.TimeoutAfter(this.wsEventLoopGroup?.ShutdownGracefullyAsync(), TimeSpan.FromSeconds(15));
                }
                catch (TimeoutException ex)
                {
            ...
            bootstrap.Group(this.parentEventLoopGroup, this.eventLoopGroup)
                ...
                .ChildHandler(
                    new ActionChannelInitializer<ISocketChannel>(
                        channel =>
                        {
                            var identityProvider = new DeviceIdentityProvider(this.authenticator, this.clientCredentialsFactory, this.metadataStore, this.clientCertAuthAllowed);
