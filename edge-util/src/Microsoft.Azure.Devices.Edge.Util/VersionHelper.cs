...
namespace Microsoft.Azure.Devices.Edge.Util
{
    ...
    {
        public static int CompareTo(this Version expectedVersion, string actualVersionString, string context)
        {
            if (string.IsNullOrWhiteSpace(actualVersionString) || !Version.TryParse(actualVersionString, out Version version))
            {
                throw new ArgumentException($"Invalid {context} version {actualVersionString ?? string.Empty}");
            }
