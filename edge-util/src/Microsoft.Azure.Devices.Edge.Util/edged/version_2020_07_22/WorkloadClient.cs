...
namespace Microsoft.Azure.Devices.Edge.Util.Edged.Version_2020_07_22
{
    using System;
    ...
    using Microsoft.Azure.Devices.Edge.Util.Edged.Version_2020_07_22.GeneratedCode;
    ...
    class WorkloadClient : WorkloadClientVersioned
    {
        ...
        public override Task<string> GetSecretAsync(string secretId)
        {
            using (HttpClient httpClient = HttpClientHelper.GetHttpClient(this.WorkloadUri))
            {
                var edgeletHttpClient = new HttpWorkloadClient(httpClient) { BaseUrl = HttpClientHelper.GetBaseUrl(this.WorkloadUri) };
                return this.Execute(() => edgeletHttpClient.GetSecretAsync(this.Version.Name, this.ModuleId, secretId), "GetSecretAsync");
            }
        }

        public override Task SetSecretAsync(string secretId, string secretValue)
        {
            using (HttpClient httpClient = HttpClientHelper.GetHttpClient(this.WorkloadUri))
            {
                var edgeletHttpClient = new HttpWorkloadClient(httpClient) { BaseUrl = HttpClientHelper.GetBaseUrl(this.WorkloadUri) };
                return this.Execute(() => edgeletHttpClient.SetSecretAsync(this.Version.Name, this.ModuleId, secretId, secretValue), "SetSecretAsync");
            }
        }
